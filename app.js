...
const SOCIAL_META = {
  ...
  // Kanal — qatorning eng oxirida, o'z avatari (robot) bilan ajralib turadi
  channel:{label:'Telegram kanal',ph:'https://t.me/KANAL', img:'assets/itecn0.jpg'}
};
...
function renderSocial(){
  ...
  w.innerHTML = Object.keys(SOCIAL_META).map(k => {
    ...
    if (!url) return '';
    const inner = SOCIAL_META[k].img
      ? `<img src="${esc(SOCIAL_META[k].img)}" alt="" loading="lazy">`
      : `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${ICONS[k]}"/></svg>`;
    return `<a class="soc ${SOCIAL_META[k].img?'soc--img':''}" href="${esc(url)}" target="_blank" rel="noopener" aria-label="${SOCIAL_META[k].label}" title="${SOCIAL_META[k].label}">${inner}</a>`;
  }).join('');
}
...
function renderContact(){
  ...
  $('#channelsWrap').innerHTML = items.map(it => `
    <a class="chan" href="${esc(it.h)}" target="_blank" rel="noopener">
      <span class="chan__i">${it.img ? `<img src="${esc(it.img)}" alt="" loading="lazy">` : `<svg viewBox="0 0 24 24">${it.i}</svg>`}</span>
